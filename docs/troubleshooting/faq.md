...
### How do I update DClaw Train?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp train --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Train without Kubernetes?

Yes, for local development:

```bash
cd dclaw-train/frontend && npm run dev
cd dclaw-train/backend && uvicorn dclaw_train.main:app --reload
```
