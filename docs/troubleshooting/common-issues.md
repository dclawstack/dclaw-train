...
# Check events
kubectl get events -n dclaw-train --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-train deployment/dclaw-train-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-train-db-credentials -n dclaw-train
   ```
